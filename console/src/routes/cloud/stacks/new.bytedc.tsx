...
type FieldType =
  | "string" | "secret" | "number" | "bool" | "multiselect"
...
export const Route = createFileRoute("/cloud/stacks/new/bytedc")({
  component: ByteDCWizard,
...
function ByteDCWizard() {
  ...
  const { data: schema, error: schemaError } = useQuery({
    queryKey: qk.stackSchema("bytedc"),
    queryFn: () => api<Schema>("GET", "/api/cloud/bytedc/schema"),
  });
  ...
  useEffect(() => {
    ...
    if (edit) {
      ...
      // Back-fill reuse toggles for legacy stacks that pre-date them
      const netKeys = ["existing_vpc_id","existing_public_subnet_id","existing_app_subnet_id","existing_data_subnet_id","existing_public_ipv4_subnet_id","existing_app_ipv4_subnet_id","existing_data_ipv4_subnet_id","existing_app_sg_id","existing_data_sg_id"];
      if (v.use_existing_network === undefined) {
        v.use_existing_network = netKeys.some(k => typeof v[k] === "string" && v[k].trim() !== "");
      }
      const natKeys = ["existing_nat_gateway_id","nat_floating_ip_id"];
      if (v.use_existing_nat === undefined) {
        v.use_existing_nat = natKeys.some(k => typeof v[k] === "string" && v[k].trim() !== "") || v.create_nat_in_existing_vpc === true || v.manage_existing_nat_snat_rules === true;
      }
      setValues(v);
  ...
  const saveMut = useMutation({
    mutationFn: async () => {
      ...
      return api("POST", "/api/cloud/stacks", { provider: "bytedc", name: stackName, values: payload });
    },
    onSuccess: () => {
  ...
  if (!schema) {
    return (
      <div className="space-y-6">
        <Breadcrumbs items={[{ label: isEdit ? `Edit · ${edit}` : "New ByteDC Stack" }]} />
  ...
  return (
    <div className="space-y-6">
      <Breadcrumbs items={[{ label: isEdit ? `Edit · ${edit}` : "New ByteDC Stack" }]} />
      <div>
        <h1 className="text-2xl font-bold">{isEdit ? "Edit Stack" : "New ByteDC Stack"}</h1>
        <p className="text-sm text-[var(--color-muted-foreground)] mt-1">
          Mirrors <code className="text-xs">IaC/opentofu-bytedc/envs/_template/variables.tf</code>. Saving creates <code className="text-xs">envs/&lt;name&gt;/</code>.
        </p>
      </div>
      {!isEdit && <ProviderIntro providerId="bytedc" />}
...
function FieldRenderer({ field, value, setValue, allValues, setOther }: {
  ...
  switch (f.type) {
    ...
    case "role_counts": {
      ...
      const flavorKey = f.flavor_key || "flavor_id";
...
function ReviewPanel({ values, stackName }: { values: Values; stackName: string }) {
  ...
  ["access_key", "secret_key", "ecs_admin_pass"].forEach(k => {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-sm text-[var(--color-muted-foreground)]">
        ...
        <code className="text-xs">IaC/opentofu-bytedc/envs/{stackName}/</code>.
      </p>
...
function parseHclLiteral(s: string): any {
  ...
  if (s.startsWith("{")) {
    ...
    inner.split("\n").forEach(line => {
      const mm = line.match(/^\s*([a-zA-Z_][\w-]*)\s*=\s*(.+?)\s*,?\s*$/);
      if (mm && mm[1]) out[mm[1]] = parseHclLiteral(mm[2] ?? "");
    });
    return out;
  }
  return s;
