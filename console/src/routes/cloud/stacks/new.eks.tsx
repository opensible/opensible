...
export const Route = createFileRoute("/cloud/stacks/new/eks")({
  component: EksWizard,
...
function EksWizard() {
  ...
  const { data: schema, error: schemaError } = useQuery({
    queryKey: qk.stackSchema("eks"),
    queryFn: () => api<Schema>("GET", "/api/cloud/eks/schema"),
  });
  ...
  const saveMut = useMutation({
    mutationFn: async () => {
      ...
      return api("POST", "/api/cloud/stacks", { provider: "eks", name: stackName, values: payload });
    },
    onSuccess: () => {
  ...
  if (!schema) {
    return (
      <div className="space-y-6">
        <Breadcrumbs items={[{ label: isEdit ? `Edit · ${edit}` : "New EKS Stack" }]} />
  ...
  return (
    <div className="space-y-6">
      <Breadcrumbs items={[{ label: isEdit ? `Edit · ${edit}` : "New EKS Stack" }]} />
      <div>
        <h1 className="text-2xl font-bold">{isEdit ? "Edit Stack" : "New EKS Stack"}</h1>
        <p className="text-sm text-[var(--color-muted-foreground)] mt-1">
          Mirrors <code className="text-xs">IaC/opentofu-eks/envs/_template/variables.tf</code>. Saving creates <code className="text-xs">envs/&lt;name&gt;/</code>.
        </p>
      </div>
      {!isEdit && <ProviderIntro providerId="eks" />}
...
function ReviewPanel({ values, stackName }: { values: Values; stackName: string }) {
  ...
  ["aws_access_key", "aws_secret_key"].forEach(k => {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-sm text-[var(--color-muted-foreground)]">
        ...
        <code className="text-xs">IaC/opentofu-eks/envs/{stackName}/</code>.
      </p>
