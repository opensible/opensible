...
export const Route = createFileRoute("/cloud/stacks/new/gcp")({
  component: GcpWizard,
...
function GcpWizard() {
  ...
  const { data: schema, error: schemaError } = useQuery({
    queryKey: qk.stackSchema("gcp"),
    queryFn: () => api<Schema>("GET", "/api/cloud/gcp/schema"),
  });
  ...
  const saveMut = useMutation({
    mutationFn: async () => {
      ...
      return api("POST", "/api/cloud/stacks", { provider: "gcp", name: stackName, values: payload });
    },
    onSuccess: () => {
  ...
  if (!schema) {
    return (
      <div className="space-y-6">
        <Breadcrumbs items={[{ label: isEdit ? `Edit · ${edit}` : "New Google Cloud Stack" }]} />
  ...
  return (
    <div className="space-y-6">
      <Breadcrumbs items={[{ label: isEdit ? `Edit · ${edit}` : "New Google Cloud Stack" }]} />
      <div>
        <h1 className="text-2xl font-bold">{isEdit ? "Edit Stack" : "New Google Cloud Stack"}</h1>
        <p className="text-sm text-[var(--color-muted-foreground)] mt-1">
          Mirrors <code className="text-xs">IaC/opentofu-gcp/envs/_template/variables.tf</code>. Saving creates <code className="text-xs">envs/&lt;name&gt;/</code>.
        </p>
      </div>
      {!isEdit && <ProviderIntro providerId="gcp" />}
...
function ReviewPanel({ values, stackName }: { values: Values; stackName: string }) {
  ...
  ["gcp_credentials_json", "admin_password"].forEach(k => {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-sm text-[var(--color-muted-foreground)]">
        ...
        <code className="text-xs">IaC/opentofu-gcp/envs/{stackName}/</code>.
      </p>
