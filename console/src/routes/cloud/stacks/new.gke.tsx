...
export const Route = createFileRoute("/cloud/stacks/new/gke")({
  component: GkeWizard,
...
function GkeWizard() {
  ...
  const { data: schema, error: schemaError } = useQuery({
    queryKey: qk.stackSchema("gke"),
    queryFn: () => api<Schema>("GET", "/api/cloud/gke/schema"),
  });
  ...
  const saveMut = useMutation({
    mutationFn: async () => {
      ...
      return api("POST", "/api/cloud/stacks", { provider: "gke", name: stackName, values: payload });
    },
    onSuccess: () => {
  ...
  if (!schema) {
    return (
      <div className="space-y-6">
        <Breadcrumbs items={[{ label: isEdit ? `Edit · ${edit}` : "New GKE Stack" }]} />
  ...
  return (
    <div className="space-y-6">
      <Breadcrumbs items={[{ label: isEdit ? `Edit · ${edit}` : "New GKE Stack" }]} />
      <div>
        <h1 className="text-2xl font-bold">{isEdit ? "Edit Stack" : "New GKE Stack"}</h1>
        <p className="text-sm text-[var(--color-muted-foreground)] mt-1">
          Mirrors <code className="text-xs">IaC/opentofu-gke/envs/_template/variables.tf</code>. Saving creates <code className="text-xs">envs/&lt;name&gt;/</code>.
        </p>
      </div>
      {!isEdit && <ProviderIntro providerId="gke" />}
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-sm text-[var(--color-muted-foreground)]">
        ...
        <code className="text-xs">IaC/opentofu-gke/envs/{stackName}/</code>.
      </p>
