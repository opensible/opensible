...
export const Route = createFileRoute("/cloud/stacks/new/hetzner")({
  component: HetznerWizard,
...
function HetznerWizard() {
  ...
  const { data: schema, error: schemaError } = useQuery({
    queryKey: qk.stackSchema("hetzner"),
    queryFn: () => api<Schema>("GET", "/api/cloud/hetzner/schema"),
  });
  ...
  const saveMut = useMutation({
    mutationFn: async () => {
      ...
      return api("POST", "/api/cloud/stacks", { provider: "hetzner", name: stackName, values: payload });
    },
    onSuccess: () => {
  ...
  if (!schema) {
    return (
      <div className="space-y-6">
        <Breadcrumbs items={[{ label: isEdit ? `Edit · ${edit}` : "New Hetzner Cloud Stack" }]} />
  ...
  return (
    <div className="space-y-6">
      <Breadcrumbs items={[{ label: isEdit ? `Edit · ${edit}` : "New Hetzner Cloud Stack" }]} />
      <div>
        <h1 className="text-2xl font-bold">{isEdit ? "Edit Stack" : "New Hetzner Cloud Stack"}</h1>
        <p className="text-sm text-[var(--color-muted-foreground)] mt-1">
          Mirrors <code className="text-xs">IaC/opentofu-hetzner/envs/_template/variables.tf</code>. Saving creates <code className="text-xs">envs/&lt;name&gt;/</code>.
        </p>
      </div>
      {!isEdit && <ProviderIntro providerId="hetzner" />}
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-sm text-[var(--color-muted-foreground)]">
        ...
        <code className="text-xs">IaC/opentofu-hetzner/envs/{stackName}/</code>.
      </p>
