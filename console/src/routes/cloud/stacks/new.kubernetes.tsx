...
function iconForGroup(g: { title: string; icon?: string }): LucideIcon {
  ...
  if (/workload|boxes/.test(key)) return Boxes;
  if (/ingress|network/.test(key)) return Network;
  if (/addon|helm|cog/.test(key)) return Cog;
...
export const Route = createFileRoute("/cloud/stacks/new/kubernetes")({
  component: KubernetesWizard,
...
function KubernetesWizard() {
  ...
  const { data: schema, error: schemaError } = useQuery({
    queryKey: qk.stackSchema("kubernetes"),
    queryFn: () => api<Schema>("GET", "/api/cloud/kubernetes/schema"),
  });
  ...
  const saveMut = useMutation({
    mutationFn: async () => {
      ...
      if (payload.workloads == null || payload.workloads === "") payload.workloads = [];
      if (isEdit && edit) {
        return api("PUT", `/api/cloud/stacks/${encodeURIComponent(edit)}`, { values: payload });
      }
      return api("POST", "/api/cloud/stacks", { provider: "kubernetes", name: stackName, values: payload });
    },
    onSuccess: () => {
  ...
  if (!schema) {
    return (
      <div className="space-y-6">
        <Breadcrumbs items={[{ label: isEdit ? `Edit · ${edit}` : "New Kubernetes Stack" }]} />
  ...
  return (
    <div className="space-y-6">
      <Breadcrumbs items={[{ label: isEdit ? `Edit · ${edit}` : "New Kubernetes Stack" }]} />
      <div>
        <h1 className="text-2xl font-bold">{isEdit ? "Edit Stack" : "New Kubernetes Stack"}</h1>
        <p className="text-sm text-[var(--color-muted-foreground)] mt-1">
          Mirrors <code className="text-xs">IaC/opentofu-kubernetes/envs/_template/variables.tf</code>. Saving creates <code className="text-xs">envs/&lt;name&gt;/</code>.
        </p>
      </div>
      {!isEdit && <ProviderIntro providerId="kubernetes" />}
        ...
        <FieldWrap label={f.label} required={f.required} help={f.help}>
          {f.multiline ? (
            <textarea
              ...
              spellCheck={false}
              autoComplete="off"
              className="w-full font-mono text-xs p-3 rounded-md border border-[var(--color-border)] bg-[var(--color-background)]"
...
function ReviewPanel({ values, stackName }: { values: Values; stackName: string }) {
  ...
  ["kubeconfig", "cluster_ca_cert", "cluster_token"].forEach(k => {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-sm text-[var(--color-muted-foreground)]">
        ...
        <code className="text-xs">IaC/opentofu-kubernetes/envs/{stackName}/</code>.
      </p>
