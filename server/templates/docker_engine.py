...
def render(values: Dict[str, Any], targets: Dict[str, Any]) -> str:
    ...
    parts = [
        ...
        *vars_files_lines(parse_vault_files(values.get("vault_files"))),
        ...
        "      ansible.builtin.apt:",
        "        name: [ca-certificates, curl, gnupg, lsb-release, iptables, kmod]",
        "        state: present",
        "        update_cache: true",
        "      when: ansible_os_family == 'Debian'",
        ...
        "        mode: '0755'",
        "      when: ansible_os_family == 'Debian'",
        "    - name: Configure Docker daemon (log rotation, live-restore)",
        ...
        *pkg_lines,
        ...
        "            enabled: true",
        "      rescue:",
        "        - name: Collect docker start failure logs",
        "          ansible.builtin.command: journalctl -u docker.service -n 50 --no-pager",
        ...
        "        - name: Fallback — disable iptables integration (LXC/VPS kernels)",
        "          ansible.builtin.copy:",
        "            dest: /etc/docker/daemon.json",
        "            mode: '0644'",
        "            content: |",
        ...
        "          ansible.builtin.systemd:",
        "            daemon_reload: true",
        "        - name: Start and enable docker (fallback)",
        "          ansible.builtin.service:",
        "            name: docker",
        "            state: started",
        "            enabled: true",
