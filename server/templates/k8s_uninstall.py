...
def _render_tasks(values: Dict[str, Any]) -> str:
    ...
    lines.append("  - name: Stop and disable Kubernetes services")
    lines.append("    ansible.builtin.systemd:")
    lines.append("      name: '{{ item }}'")
    lines.append("      state: stopped")
    lines.append("      enabled: false")
    lines.append("    loop:")
    for svc in ["kubelet", "containerd", "k3s", "k3s-agent", "kubectl-proxy"]:
        lines.append(f"      - {svc}")
    lines.append("    ignore_errors: true")
    lines.append("")
    ...
    if remove_packages:
        ...
        lines.append("    ansible.builtin.apt:")
        lines.append("      name:")
        for pkg in ["kubelet", "kubeadm", "kubectl", "kubernetes-cni"]:
            lines.append(f"        - {pkg}")
        lines.append("      state: absent")
        lines.append("      purge: true")
        lines.append("      autoremove: true")
        lines.append("    ignore_errors: true")
        lines.append("")
    ...
    if remove_containerd:
        ...
        lines.append("    ansible.builtin.apt:")
        lines.append("      name:")
        for pkg in ["containerd", "containerd.io", "runc"]:
            lines.append(f"        - {pkg}")
        lines.append("      state: absent")
        lines.append("      purge: true")
        lines.append("      autoremove: true")
    ...
    lines.append('      msg: "Kubernetes purge completed on {{ inventory_hostname }} (cluster_type={{ cluster_type }})"')
    lines.append("")
